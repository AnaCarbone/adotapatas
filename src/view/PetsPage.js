...
class PetsPage extends Component {
  ...
  render() {
    ...
    const {
      pets, rowsPerPage, page, isFemale, amountAnimals,
    } = this.state;
    return (
      <div>
        <ButtonAppBar />
        ...
        <div className={classes.grid}>
          <GridList cellHeight={360} cols="3">
            {pets.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((pet) => {
              ...
              const gender = isFemale ? classes.female : classes.male;
              return (
                <GridListTile>
                  <Card className={classes.card} onClick={this.popUp}>
                    <CardHeader
                      avatar={(
                        <Avatar aria-label="recipe" className={gender}>
                          {pet.gender}
                        </Avatar>
                      )}
                      title={pet.type}
